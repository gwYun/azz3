...
## /plan-eng-review 2026-07-29에서 도출된 항목 (카카오 로그인 플랜)

### 저장빌드 계정 동기화 (로그인에 실질 가치 부여)

**무엇:** `web/lib/storage.ts`가 localStorage 키 `azz3.builds.v1`에 저장하는 빌드를, 로그인 계정 스코프 DB 테이블(+RLS)로 승격해 기기 간 동기화.

**왜:** 카카오 로그인 플랜(`docs/kakao-auth-payment-plan.md`)에서 Phase 1은 로그인만 붙이는데, 아웃사이드 보이스(OV1) 지적대로 로그인 뒤에 보여줄 게 이름표뿐이라 유저 체감 가치가 0 — 도그푸딩이 불가능한 순수 인프라. 저장빌드 동기화를 붙이면 로그인이 결제 전제조건을 넘어 "기기 바꿔도 내 빌드가 남는" 실기능이 됨.

**장점:** 로그인이 도그푸딩 가능한 기능이 됨; Phase 2 결제 스키마(profiles/orders)와 같은 시점에 DB를 도입하면 자연스러움.

**단점:** Phase 1 스코프·diff 확대, DB 테이블+RLS 설계가 로그인 PR로 당겨짐(두 번 확정한 "Phase 1 최소 스코프" 결정과 충돌).

**컨텍스트:** `web/lib/storage.ts` (localStorage 저장/로드 로직), Supabase Postgres + RLS. 로그인 세션의 user id로 빌드 소유권 키잉.

**의존성 / 결정 필요:** Phase 1에 당길지 vs Phase 2 결제 스키마와 묶을지. `/plan-eng-review`에서 사용자가 "지금 결정 보류(TODO)"로 선택 — 로그인 먼저 띄우고 이후 사이클에서 판단.
