...
export function CompareView({ a, b, info, result, loading, error, onClose }: Props) {
  ...
  return (
    <section
      className="mt-8 rounded-lg border border-neutral-200 bg-white p-6 shadow-elevated"
      aria-labelledby="compare-title"
    >
      <div className="flex items-start justify-between gap-4">
        <h2 id="compare-title" className="font-display text-xl font-semibold text-neutral-900">
          {t("compare.title")}
        </h2>
      ...
      <div className="mt-6 grid grid-cols-3 gap-4 border-y border-neutral-200 py-4">
        <div className="text-center">
          <div className="text-xs font-semibold uppercase tracking-wide text-neutral-500">{a.name}</div>
          <div className="mt-1 font-display text-2xl font-semibold tabular-nums text-neutral-900">
            {result ? euro(result.a.predicted_fee_eur, locale) : euro(a.predicted_fee_eur, locale)}
          </div>
        </div>
        <div className="flex items-center justify-center text-xs font-semibold uppercase tracking-wide text-neutral-400">
          vs
        </div>
        <div className="text-center">
          <div className="text-xs font-semibold uppercase tracking-wide text-neutral-500">{b.name}</div>
          <div className="mt-1 font-display text-2xl font-semibold tabular-nums text-neutral-900">
      ...
      {result && result.deciding_group ? (
        <p className="mt-4 rounded bg-accent-tint p-3 text-sm text-neutral-800">
          {(() => {
      ...
      {loading && !result ? (
        <p className="mt-6 text-sm text-neutral-500">{t("loading")}</p>
      ) : null}
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-neutral-200 font-mono tabular-nums">
            {info.features.map((feat) => {
              ...
              return (
                <tr key={feat}>
                  <td className="px-3 py-1.5 font-sans text-xs text-neutral-700">{t(fullKey)}</td>
                  <td
                    className={
                      "px-3 py-1.5 text-right " +
                      (aBetter ? "text-accent-dark font-semibold" : tied ? "text-neutral-500" : "text-neutral-700")
                    }
                  >
                    {aBetter ? "▲ " : ""}
                    {num(va, decs)}
                  ...
                  <td
                    className={
                      "px-3 py-1.5 text-right " +
                      (bBetter ? "text-accent-dark font-semibold" : tied ? "text-neutral-500" : "text-neutral-700")
                    }
                  >
                    {bBetter ? "▲ " : ""}
                    {num(vb, decs)}
