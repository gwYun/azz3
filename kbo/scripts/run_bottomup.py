...
Builds every team from its real players (value → salary → roster/lineup → team rs/ra),
...
def assemble(ratings, values, out, bt: pd.DataFrame, sims, seed) -> dict:
    ...
    return {
        ...
        "method": "bottom-up player-level (value → salary → lineup/playing-time → team rs/ra)",
        "params": _SIM_PARAMS,
...
def write_report(payload: dict) -> str:
    ...
    L.append(f"\n## 방법론\n\n- **데이터**: 백테스트=공개 전체 로스터(LOPES, 2010-2020, 박스스코어 디코딩), "
             "2026=KBO /Record 규정선수+팀합계 앵커 벤치. statiz 미사용(robots 차단).\n")
    L.append("- **파이프라인**: 선수 스탯 → 가치(wOBA/wRC+/FIP/WAR) → 추정연봉 → 라인업·출전 preset·"
             "감독 전술 → 팀 공격/수비 득점 → 승리환경(연봉+시너지) → 144경기+한국시리즈 몬테카를로.\n")
